...
#Global variable #3 >> GoogleAdminEmail
$tmpName = @'
GoogleAdminEmail
'@ 
$tmpValue = @'
...
#Global variable #4 >> GoogleServiceAccountEmail
$tmpName = @'
...
<# Begin: DataSource "Google-Get-Group" #>
$tmpPsScript = @'
# Variables configured in form
$searchValue = $datasource.searchValue
...
$p12CertificatePassword = $GoogleP12CertificatePassword
$serviceAccountEmail = $GoogleServiceAccountEmail
$userId = $GoogleAdminEmail # Email address of admin with permissions to manage groups.
...
try {
    ...
    #region Get Google Groups
    # Google docs: https://developers.google.com/admin-sdk/directory/reference/rest/v1/groups/list
    $actionMessage = "querying Google Groups"

    $googleGroups = [System.Collections.ArrayList]@()

    $getGoogleGroupsSplatParams = @{
        customer = "my_customer"
    ...
    do {
        $getGoogleGroupsSplatParams = @{
            ...
            Headers     = $headers
            Body        = $getGoogleGroupsSplatParams
            Method      = "GET"
            Verbose     = $false
            ErrorAction = "Stop"
        }
        if (-not[string]::IsNullOrEmpty($getGoogleGroupsResponse.nextPageToken)) {
            $getGoogleGroupsSplatParams['pageToken'] = $getGoogleGroupsResponse.nextPageToken
        }

        $getGoogleGroupsResponse = $null
        $getGoogleGroupsResponse = Invoke-RestMethod @getGoogleGroupsSplatParams
    
        if ($getGoogleGroupsResponse.groups -is [array]) {
            [void]$googleGroups.AddRange($getGoogleGroupsResponse.groups)
        }
        else {
            [void]$googleGroups.Add($getGoogleGroupsResponse.groups)
        }
    } while (-not[string]::IsNullOrEmpty($getGoogleGroupsResponse.nextPageToken))

    Write-Information "Queried Google Groups. Result count: $(($googleGroups | Measure-Object).Count)"
    #endregion Get Google Groups

    if(($googleGroups | Measure-Object).Count -ge 1){
        #region Send results to HelloID
        $actionMessage = "sending results to HelloID"

        $googleGroups | ForEach-Object {
            $outputObject = [PSCustomObject]@{
                id          = $_.id
                name        = $_.name
                email       = $_.email
...
'@ 
$tmpModel = @'
[{"key":"name","type":0},{"key":"email","type":0},{"key":"description","type":0}]
'@ 
$tmpInput = @'
[{"description":null,"translateDescription":false,"inputFieldType":1,"key":"searchValue","type":0,"options":1}]
'@ 
...
<# End: DataSource "Google-Get-Group" #>
<# End: HelloID Data sources #>

<# Begin: Dynamic Form "Google Group - Add or Remove User" #>
$tmpSchema = @"
[{"key":"formRow1","templateOptions":{},"fieldGroup":[{"key":"searchfieldGroup","templateOptions":{"label":"Search group (enter exact email address)","placeholder":"Exact email address of group","required":true,"pattern":"^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$"},"type":"input","summaryVisibility":"Hide element","requiresTemplateOptions":true,"requiresKey":true,"requiresDataSource":false},{"key":"searchfieldUser","templateOptions":{"label":"Search user (enter exact email address)","required":true,"placeholder":"Exact email address of user","pattern":"^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$"},"type":"input","summaryVisibility":"Hide element","requiresTemplateOptions":true,"requiresKey":true,"requiresDataSource":false}],"type":"formrow","requiresTemplateOptions":true,"requiresKey":true,"requiresDataSource":false},{"key":"formRow","templateOptions":{},"fieldGroup":[{"key":"group","templateOptions":{"label":"Select group","required":true,"grid":{"columns":[{"headerName":"Email","field":"email"},{"headerName":"Name","field":"name"},{"headerName":"Description","field":"description"}],"height":300,"rowSelection":"single"},"dataSourceConfig":{"dataSourceGuid":"$dataSourceGuid_0","input":{"propertyInputs":[{"propertyName":"searchValue","otherFieldValue":{"otherFieldKey":"searchfieldGroup"}}]}},"useFilter":false,"allowCsvDownload":true,"defaultSelectorProperty":"email","useDefault":true},"hideExpression":"!model[\"searchfieldGroup\"]","type":"grid","summaryVisibility":"Show","requiresTemplateOptions":true,"requiresKey":true,"requiresDataSource":true},{"key":"user","templateOptions":{"label":"Select user","required":true,"grid":{"columns":[{"headerName":"Primary Email","field":"primaryEmail"},{"headerName":"Full Name","field":"fullName"}],"height":300,"rowSelection":"single"},"dataSourceConfig":{"dataSourceGuid":"$dataSourceGuid_1","input":{"propertyInputs":[{"propertyName":"searchValue","otherFieldValue":{"otherFieldKey":"searchfieldUser"}}]}},"useDefault":true,"searchPlaceHolder":"Search this data","allowCsvDownload":false,"defaultSelectorProperty":"primaryEmail"},"hideExpression":"!model[\"searchfieldUser\"]","type":"grid","summaryVisibility":"Show","requiresTemplateOptions":true,"requiresKey":true,"requiresDataSource":true}],"type":"formrow","requiresTemplateOptions":true,"requiresKey":true,"requiresDataSource":false},{"key":"action","templateOptions":{"label":"Action to perform","required":true,"useObjects":true,"useDataSource":false,"useFilter":false,"options":[{"value":"addUserToGroup","text":"Add user to group"},{"value":"removeUserFromGroup","text":"Remove user from group"}]},"type":"dropdown","summaryVisibility":"Show","textOrLabel":"text","requiresTemplateOptions":true,"requiresKey":true,"requiresDataSource":false}]
"@ 

$dynamicFormGuid = [PSCustomObject]@{} 
$dynamicFormName = @'
Google Group - Add or Remove User
'@ 
